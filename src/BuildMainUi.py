...
def create_menu_system(top_window):
    ...
    thes_item = tools_menu.Append(wx.ID_ANY, 'Synonym Check\tF4', 'Consult word against Big Huge Thesaurus')
    top_window.Bind(wx.EVT_MENU, top_window.synonym_check, thes_item)
    ...
    report_menu = wx.Menu()
    thing_minimal = report_menu.Append(wx.ID_ANY, 'Thing (Minimal)', 'Generate Minimal Thing Implementation Report')
    top_window.Bind(wx.EVT_MENU, top_window.minimal_thing_report, thing_minimal)
    thing_extensive = report_menu.Append(wx.ID_ANY, 'Thing (Extensive)', 'Generate Extensive Thing Implementation Report')
    top_window.Bind(wx.EVT_MENU, top_window.extensive_thing_report, thing_extensive)
    actor_report = report_menu.Append(wx.ID_ANY, 'Actor and ActorState', 'Generate Actor/ActorState Implementation Report')
    top_window.Bind(wx.EVT_MENU, top_window.actor_report, actor_report)
    tools_menu.AppendMenu(wx.ID_ANY, 'Implementation Report', report_menu)
    tools_menu.AppendSeparator()
    ...
    top_window.grayable.append(thes_item)
